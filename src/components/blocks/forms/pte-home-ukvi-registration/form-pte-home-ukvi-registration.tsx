...
export default function FormPTEHomeUKVIRegistration() {
  ...
  const getFieldsForStep = (currentStep: number): (keyof TPteHomeUkviFormSchema)[] => {
    switch (currentStep) {
                            ...
                            <RadioGroup className="flex gap-8 h-12 items-center" onValueChange={field.onChange} value={field.value}>
                              {["Male", "Female", "Other"].map(s => (
                                <div key={s} className="flex items-center space-x-2 group cursor-pointer">
                                  <RadioGroupItem value={s.toLowerCase()} id={`g-home-ukvi-${s}`} className="border-[#A11D1D]" />
                                  <Label htmlFor={`g-home-ukvi-${s}`} className="text-sm font-bold capitalize cursor-pointer">{s}</Label>
                                </div>
                              ))}
                            </RadioGroup>
                          </FormControl>
                    ...
                    <FormItem className="space-y-6">
                      ...
                      <FormControl>
                        <RadioGroup onValueChange={field.onChange} value={field.value} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {["Visa (UKVI)", "Citizenship", "Other"].map(r => (
