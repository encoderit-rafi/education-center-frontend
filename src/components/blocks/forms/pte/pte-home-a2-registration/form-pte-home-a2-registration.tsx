...
// Schema
import { PteHomeA2Schema, type TPteHomeA2Schema } from "./_type";
...
export default function FormPTEHomeA2Registration({ examId: initialExamId }: FormProps = {}) {
  ...
  const activeExam = initialExamId
    ...
    : examsList.find((e: any) => e.slug === "pte-home-a2");
  ...
  const calculateTotal = () => {
    const baseFee = activeExam?.examFee ? parseFloat(activeExam.examFee) : 1230;
  ...
  const handleFormSubmit: SubmitHandler<TPteHomeA2Schema> = async (data) => {
    if (currentStep < 3) {
      nextStep();
    } else {
      try {
