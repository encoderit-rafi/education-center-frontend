...
export function getDisplayDiscount(courseName: string, actualDiscount: number): number {
...
export function calculateCourseDiscountedPrice(
  ...
  const rawDiscount = parseFloat(String(discountValue ?? 0)) || 0;
  if (!discountType || discountType === "PERCENTAGE") {
    const effectiveDiscount = getDisplayDiscount(courseName, rawDiscount);
    return Math.round(basePrice * (1 - effectiveDiscount / 100) * 100) / 100;
  }
  return basePrice - rawDiscount;
}
