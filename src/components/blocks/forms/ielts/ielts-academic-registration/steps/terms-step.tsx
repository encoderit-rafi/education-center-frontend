...
export function TermsStep({ onNext, examFee, additionalFee }: TermsStepProps) {
  const notices: (React.ReactNode | string)[] = [
    ...
    <span key="fee">
      In the UAE, the current fee for the computer-delivered (CD) <strong className="font-semibold">IELTS Academic</strong> exam is <strong className="font-semibold"><AED className="h-[0.8em] w-auto fill-current inline-block" /> {examFee.toLocaleString()}</strong> (VAT inclusive). Candidates opting to use the <strong className="font-semibold">TEPTH</strong> registration service will incur an administrative fee of <strong className="font-semibold"><AED className="h-[0.8em] w-auto fill-current inline-block" /> {additionalFee.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</strong>, which is applied in addition to the standard CD-IELTS exam fee.
    </span>,
    ...
    "We are not responsible if we cannot run the test or fulfill our duties due to extreme situations entirely out of our hands. This includes major emergencies like natural disasters, wars, or unexpected power and technical blackout.",
  ];
