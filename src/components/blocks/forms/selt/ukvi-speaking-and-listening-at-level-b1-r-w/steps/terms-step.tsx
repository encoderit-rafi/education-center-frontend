...
export function TermsStep({ onNext, examFee, additionalFee }: TermsStepProps) {
  ...
  const notices: (React.ReactNode | string)[] = Array.from({ length: 21 }).map((_, idx) => {
    return t.rich(`selt_b1_rw.${idx}`, {
      aed_exam: () => (
        <strong className="font-semibold inline-flex items-center gap-1 whitespace-nowrap"><AED className="h-[0.8em] w-auto fill-current" /> {examFee.toLocaleString()}</strong>
      ),
      aed_additional: () => (
        <strong className="font-semibold inline-flex items-center gap-1 whitespace-nowrap"><AED className="h-[0.8em] w-auto fill-current" /> {additionalFee.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</strong>
      ),
