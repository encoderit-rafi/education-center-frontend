...
export default function FormSELTB1RWRegistration({
  ...
  const activeExam = initialExamId
    ...
    : examsList.find(
      (e: any) =>
        ...
        e.slug === "ukvi-speaking-listening-reading-and-writing-level-b1" ||
        e.slug === "ukvi-speaking-and-listening-at-level-b1-r-w" ||
        e.slug === "selt-b1-r-w" ||
        ((e.name?.toLowerCase().includes("b1") || e.name?.toLowerCase().includes("level b1")) &&
          (e.name?.toLowerCase().includes("reading") || e.name?.toLowerCase().includes("writing"))),
    );
      ...
      try {
        ...
        const compiledPayload = compileBookingPayload({
          ...
          allFormData: {
            ...
            level_name: activeExam?.name || "SELT B1 (R/W)",
