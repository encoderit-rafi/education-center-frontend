...
export default function FormSELTB1Registration({
  ...
  const activeExam = initialExamId
    ...
    : examsList.find(
      (e: any) =>
        e.slug === "ukvi-speaking-and-listening-at-level-b1" ||
        e.slug === "ukvi-speaking-and-listening-level-b1" ||
        e.slug === "selt-b1" ||
        ((e.name?.toLowerCase().includes("level b1") || e.name?.toLowerCase().includes("level-b1")) &&
  ...
  const titleObj = useRegistrationTitle(activeExam?.slug || "ukvi-speaking-and-listening-at-level-b1", activeExam);
      ...
      try {
        ...
        const compiledPayload = compileBookingPayload({
          ...
          allFormData: {
            ...
            level_name: activeExam?.name || "SELT B1",
