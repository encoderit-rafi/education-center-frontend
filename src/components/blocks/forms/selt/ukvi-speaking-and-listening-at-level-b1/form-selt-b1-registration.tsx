...
export default function FormSELTB1Registration({ examId: initialExamId }: FormProps = {}) {
  const [currentStep, setCurrentStep] = useState(0); 
  const initialId = "selt-b1";

  const { data: examDetailResponse } = useQuery({
    queryKey: ["exam-detail", "ukvi-speaking-and-listening-at-level-b1"],
    queryFn: async () => {
      const response = await api.get("/exams/ukvi-speaking-and-listening-at-level-b1");
  ...
  const examName = examDetailResponse?.data?.name || Object.values(EXAM_IDS_DATA).find(e => e.id === initialId)?.name || "SELT B1";
