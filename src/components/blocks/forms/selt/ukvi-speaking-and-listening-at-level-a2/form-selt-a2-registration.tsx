...
export default function FormSELTA2Registration({ examId: initialExamId }: FormProps = {}) {
  const [currentStep, setCurrentStep] = useState(0); 
  const initialId = "selt-a2";

  const { data: examDetailResponse } = useQuery({
    queryKey: ["exam-detail", "ukvi-speaking-and-listening-at-level-a2"],
    queryFn: async () => {
      const response = await api.get("/exams/ukvi-speaking-and-listening-at-level-a2");
  ...
  const examName = examDetailResponse?.data?.name || Object.values(EXAM_IDS_DATA).find(e => e.id === initialId)?.name || "SELT A2";
