...
export default function FormSELTC1Registration({ examId: initialExamId }: FormProps = {}) {
  ...
  const { data: examDetailResponse } = useQuery({
    queryKey: ["exam-detail", "ukvi-speaking-listening-reading-and-writing-at-level-c1"],
    queryFn: async () => {
      const response = await api.get("/exams/ukvi-speaking-listening-reading-and-writing-at-level-c1");
