...
export default function FormSELTC2Registration({ examId: initialExamId }: FormProps = {}) {
  ...
  const { data: examDetailResponse } = useQuery({
    queryKey: ["exam-detail", "ukvi-speaking-listening-reading-and-writing-at-level-c2"],
    queryFn: async () => {
      const response = await api.get("/exams/ukvi-speaking-listening-reading-and-writing-at-level-c2");
  ...
  const examName = examDetailResponse?.data?.name || Object.values(EXAM_IDS_DATA).find(e => e.id === initialId)?.name || "SELT C2";
