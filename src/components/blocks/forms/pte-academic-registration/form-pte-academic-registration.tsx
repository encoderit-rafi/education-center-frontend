...
import { courses as globalCourses } from "@/lib/data";
...
export default function FormPTEAcademicRegistration() {
  ...
  const watchReason = watch("reasonForTaking");
  ...
  const getFieldsForStep = (currentStep: number): (keyof TPteAcademicFormSchema)[] => {
    switch (currentStep) {
      ...
      case 3: return ["idType", "idCountryOfIssue", "documentNumber", "passportCopy"];
      case 4: return ["homeLanguage", "planningCountry", "currentSituation", "reasonForTaking", "studyLevel", "occupationSector", "referralSource", "takenBefore", "takenWithinTwoYears", "hasExistingAccount", "scoreAllocationAU", "scoreAllocationNZ", "dataSharingAgreed", "bookingTermsAgreed", "marketingConsent"];
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-12">
                ...
                <FormItem>
                  <FormControl>
                    <div className="space-y-4">
                      {/* Independent field trackers for Date and Time */}
                      <FormField
                        control={control}
                        name="examDate"
              ...
              <div className="space-y-12 animate-in fade-in slide-in-from-right-8 duration-500">
                <div className="bg-orange-50/50 border border-orange-100 rounded-2xl p-6 md:p-8 space-y-4">
                  <h3 className="text-gray-900 font-bold flex items-center gap-2 text-sm uppercase tracking-widest">
                    <Info className="w-4 h-4 text-orange-500" /> Please note:
                  </h3>
                  <p className="text-gray-600 text-sm leading-relaxed font-medium">To continue with this booking you will need a valid ID and a payment card.</p>
                </div>
                  ...
                  <div className="grid grid-cols-1 gap-8">
                    <FormField control={control} name="givenNames" render={({ field }) => (
                      <FormItem className="space-y-2">
                        ...
                        <FormControl><Input placeholder="As per passport" className="rounded-lg bg-white border-slate-200 h-12" {...field} /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
                            ...
                            <RadioGroup className="flex gap-8 h-12 items-center" onValueChange={field.onChange} value={field.value}>
                              {["Male", "Female", "Other"].map(s => (
                                <div key={s} className="flex items-center space-x-2 group cursor-pointer">
                                  <RadioGroupItem value={s.toLowerCase()} id={`sex-${s}`} className="border-[#A11D1D] text-[#A11D1D]" />
                ...
                <div className="space-y-8 pt-8 border-t border-slate-100">
                  <h2 className="text-xl font-black text-gray-900 tracking-tight">Birth information</h2>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <FormField control={control} name="placeOfBirth" render={({ field }) => (
                      <FormItem className="space-y-2">
                        <FormLabel className="text-xs font-black uppercase tracking-widest text-gray-500">Town / City of Birth:*</FormLabel>
                        <FormControl><Input placeholder="As per passport" className="h-12 rounded-lg" {...field} /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
                    <FormField control={control} name="countryOfBirth" render={({ field }) => (
                      <FormItem className="space-y-2">
                        ...
                        <FormControl>
                          <CountryDropdown placeholder="Select country" value={field.value} onChange={(c) => field.onChange(c.name)} className="h-12" />
                        </FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
                ...
                <div className="space-y-8 pt-8 border-t border-slate-100">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <FormField control={control} name="countryOfResidence" render={({ field }) => (
                      <FormItem className="space-y-2">
                        <FormLabel className="text-xs font-black uppercase tracking-widest text-gray-500">Country of Residence:*</FormLabel>
                        <FormControl>
                          <CountryDropdown placeholder="Select country" value={field.value} onChange={(c) => field.onChange(c.name)} className="h-12" />
                        </FormControl>
                        <FormMessage />
                ...
                <div className="space-y-8 pt-8 border-t border-slate-100">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <FormField control={control} name="emailUsername" render={({ field }) => (<FormItem className="space-y-2"><FormLabel className="text-xs font-black uppercase tracking-widest text-gray-500">Email:*</FormLabel><FormControl><Input placeholder="example@email.com" className="h-12 rounded-lg" {...field} /></FormControl><FormMessage /></FormItem>)} />
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    <FormField control={control} name="address" render={({ field }) => (
                      <FormItem className="space-y-2">
                        <FormLabel className="text-xs font-black uppercase tracking-widest text-gray-500">Full Address:*</FormLabel>
                        <FormControl><Input placeholder="Building, Street, Area" className="h-12 rounded-lg" {...field} /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
                    <FormField control={control} name="city" render={({ field }) => (
                      <FormItem className="space-y-2">
                        ...
                        <FormControl><Input placeholder="Current city" className="h-12 rounded-lg" {...field} /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
                  </div>
                ...
                <div className="space-y-8 pt-8 border-t border-slate-100">
                  <h2 className="text-xl font-black text-gray-900 tracking-tight">Booking readiness</h2>
                    ...
                    <FormItem className="space-y-4">
                      ...
                      <FormControl>
                        <RadioGroup onValueChange={field.onChange} value={field.value} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {["I am ready to book now", "I am planning to book soon"].map(option => (
                            <div key={option} onClick={() => field.onChange(option)} className={cn("flex items-center space-x-4 p-4 rounded-xl border border-slate-100 hover:border-slate-200 transition-all cursor-pointer group bg-white", field.value === option ? "border-[#A11D1D] bg-[#A11D1D]/5" : "")}>
                              <RadioGroupItem value={option} id={`ready-${option}`} className="border-slate-300 data-[state=checked]:border-[#A11D1D] data-[state=checked]:text-[#A11D1D]" />
                              <Label htmlFor={`ready-${option}`} className="text-sm font-bold text-gray-600 cursor-pointer group-hover:text-[#A11D1D] transition-colors">{option}</Label>
                            </div>
                          ))}
                        </RadioGroup>
                      </FormControl>
                      ...
                      <FormControl>
                        <div className="relative group h-40 rounded-2xl border-2 border-dashed border-slate-200 hover:border-[#A11D1D]/50 flex flex-col items-center justify-center gap-2 cursor-pointer">
                          ...
                          {field.value && <div className="absolute inset-0 bg-white flex items-center justify-center p-4">
                            <span className="text-[10px] font-bold text-slate-600">{(field.value as File).name}</span>
                            <button type="button" onClick={() => field.onChange(undefined)} className="absolute top-2 right-2 p-1 bg-red-100 text-[#A11D1D] rounded-full"><X className="w-3 h-3" /></button>
                          </div>}
                        </div>
                      </FormControl>
                      <FormMessage />
              ...
              <div className="space-y-12 animate-in fade-in slide-in-from-right-8 duration-500">
                <div className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                    ...
                    <FormField control={control} name="planningCountry" render={({ field }) => (<FormItem className="space-y-2"><FormLabel className="text-xs font-black uppercase tracking-widest text-gray-500">Destination country?*</FormLabel><FormControl><CountryDropdown placeholder="Select country" value={field.value} onChange={(c) => field.onChange(c.name)} className="h-12" /></FormControl><FormMessage /></FormItem>)} />
                  </div>
                  ...
                  <FormField control={control} name="reasonForTaking" render={({ field }) => (
                    <FormItem className="space-y-6">
                      <FormLabel className="text-sm font-bold text-gray-700">Why are you taking PTE Academic?*</FormLabel>
                      <FormControl>
                        <RadioGroup onValueChange={field.onChange} value={field.value} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {["Study", "Migration", "Work", "Other"].map(r => (
                  ...
                  {watchReason === "study" && (
                    <FormField control={control} name="studyLevel" render={({ field }) => (
                      <FormItem className="space-y-2 animate-in fade-in slide-in-from-top-4 duration-300">
                        <FormLabel className="text-xs font-black uppercase tracking-widest text-gray-500">Study Level?*</FormLabel>
                        <FormControl><SearchableDropdown options={[{ label: "Undergraduate", value: "undergraduate" }, { label: "Postgraduate", value: "postgraduate" }, { label: "PhD", value: "phd" }, { label: "Foundation", value: "foundation" }, { label: "Other", value: "other" }]} placeholder="Select level" value={field.value} onChange={field.onChange} className="h-12" /></FormControl>
                        <FormMessage />
                      </FormItem>
                    )} />
                  )}
                ...
                <div className="space-y-8 pt-8 border-t border-slate-100">
                  ...
                  <FormField control={control} name="takenBefore" render={({ field }) => (
                    <FormItem className="space-y-6">
                      <FormLabel className="text-sm font-bold text-gray-700">Have you taken PTE Academic before?*</FormLabel>
                      <FormControl>
                        <RadioGroup onValueChange={field.onChange} value={field.value} className="grid grid-cols-1 md:grid-cols-2 gap-4">
                          {[
                            { label: "Yes", value: "yes" },
                ...
                <div className="space-y-8 pt-8 border-t border-slate-100">
                  <h2 className="text-xl font-black text-gray-900 tracking-tight">Score allocation</h2>
                  <div className="bg-slate-50 p-8 rounded-2xl border border-slate-100 space-y-6">
                    <p className="text-sm font-medium text-gray-600 leading-relaxed">PTE Academic scores can be used for migration to Australia and New Zealand. Would you like Pearson to send your scores to these governments?</p>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                      <FormField control={control} name="scoreAllocationAU" render={({ field }) => (
                        <FormItem className="flex items-start space-x-4 p-4 rounded-xl bg-white border border-slate-100">
                          <FormControl><Checkbox checked={field.value} onCheckedChange={field.onChange} className="data-[state=checked]:bg-[#A11D1D]" /></FormControl>
                          <Label className="text-sm font-bold text-gray-700 cursor-pointer">Department of Home Affairs (DHA), Australia</Label>
                        </FormItem>
                      )} />
                      <FormField control={control} name="scoreAllocationNZ" render={({ field }) => (
                        <FormItem className="flex items-start space-x-4 p-4 rounded-xl bg-white border border-slate-100">
                          <FormControl><Checkbox checked={field.value} onCheckedChange={field.onChange} className="data-[state=checked]:bg-[#A11D1D]" /></FormControl>
                          <Label className="text-sm font-bold text-gray-700 cursor-pointer">Immigration New Zealand (INZ)</Label>
                        </FormItem>
                      )} />
                    </div>
                  </div>
                  ...
                  <div className="space-y-6">
                    ...
                    <FormField control={control} name="bookingTermsAgreed" render={({ field }) => (
                      <FormItem className="flex items-start space-x-4 p-6 rounded-2xl bg-[#A11D1D]/5 border border-[#A11D1D]/10">
                        <FormControl><Checkbox checked={field.value} onCheckedChange={field.onChange} className="data-[state=checked]:bg-[#A11D1D] mt-1" /></FormControl>
                        <div className="space-y-1">
                          <Label className="text-sm font-bold text-gray-900 cursor-pointer">I have read and agree to the PTE Academic Test Taker Handbook and Booking Terms and Conditions.*</Label>
                        </div>
                      </FormItem>
                    )} />
                        ...
                        <RadioGroup onValueChange={field.onChange} value={field.value} className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          {pteCourses.map(course => (
                            <div key={course.id} onClick={() => field.onChange(course.id)} className={cn("relative p-6 rounded-2xl border-2 transition-all cursor-pointer group flex flex-col items-center text-center gap-4", field.value === course.id ? "border-[#A11D1D] bg-[#A11D1D]/5" : "border-slate-100 bg-white hover:border-slate-200")}>
                              ...
                              <div>
                                <h4 className="font-black text-gray-900 mb-1">{course.label}</h4>
                                <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">AED {course.price.toLocaleString()}</p>
                              </div>
                ...
                <div className="space-y-8">
                  ...
                  <div className="bg-slate-50 rounded-2xl p-8 space-y-6">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-black text-gray-900">AED {EXAM_FEE.toLocaleString()}.00</span>
                    </div>
                    {coursePrice > 0 && (
                      <div className="flex justify-between text-sm">
                        <span className="text-gray-500 font-medium">Preparation Course:</span>
                        <span className="font-black text-gray-900">AED {coursePrice.toLocaleString()}.00</span>
                      </div>
                    )}
                    <div className="flex justify-between text-sm pt-4 border-t border-slate-200 font-black">
                      <span className="text-lg text-gray-900">Total:</span>
                      <span className="text-2xl text-[#A11D1D]">AED {total.toLocaleString(undefined, { minimumFractionDigits: 2 })}</span>
                    </div>
                  </div>
                  <FormField control={control} name="infoCorrect" render={({ field }) => (
                    <FormItem className="flex items-start space-x-4 p-6 rounded-2xl bg-[#A11D1D]/5 border border-[#A11D1D]/10">
                      <FormControl><Checkbox checked={field.value} onCheckedChange={field.onChange} className="data-[state=checked]:bg-[#A11D1D] mt-1" /></FormControl>
                      <Label className="text-sm font-bold text-gray-700 leading-relaxed cursor-pointer">I confirm that all information is correct.</Label>
                    </FormItem>
                  )} />
                </div>
              </div>
            ...
            {step < 8 && (
              <div className="pt-12 border-t border-slate-100 flex flex-col md:flex-row justify-between items-center gap-6 mt-8">
                {step > 1 && (
                  <button onClick={() => setStep(step - 1)} className="text-gray-400 font-black text-xs uppercase tracking-widest hover:text-[#A11D1D] px-4 py-2" type="button">Previous</button>
                )}
