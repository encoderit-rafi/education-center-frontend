...
import { CheckCircle2, Info, Clock, CreditCard, CalendarDays, Mail, Phone, ExternalLink } from "lucide-react";
import {
  useReactTable,
  getCoreRowModel,
  flexRender,
  ...
  columnHelper.accessor("fee", {
    ...
    cell: (info) => (
      <div className="text-right">
        <span className="text-xl font-black text-gray-900">{info.getValue()}</span>
        <span className="text-xs font-bold text-gray-400 ml-1">AED</span>
      </div>
    ),
  }),
];
      ...
      <header className="bg-white border-b border-gray-100 py-16">
        <div className="container px-6 mx-auto lg:px-24">
          <div className="max-w-4xl">
            <span className="text-xs font-black uppercase tracking-[0.2em] text-[#A11D1D] mb-3 block">Exam Proctoring Services</span>
            ...
            <p className="text-gray-500 text-base md:text-lg font-medium max-w-2xl leading-relaxed">
              Professional proctoring services for students from other institutions who need a secure and monitored environment for their exams.
            </p>
          </div>
        </div>
      </header>
          ...
          <div className="lg:col-span-7 bg-white p-10 rounded-xl border border-gray-100 shadow-sm">
            ...
            <div className="space-y-10">
              <div className="flex gap-6 group">
                <span className="text-3xl font-black text-gray-100 group-hover:text-[#A11D1D]/20 transition-colors">1.</span>
                <div>
                  <h3 className="text-base font-black text-gray-900 uppercase tracking-wide mb-2">Review Rules & Regulations</h3>
                  <p className="text-sm text-gray-500 leading-relaxed font-medium">Review the rules of your home institution carefully. Pay attention to deadlines, requirements, and rules for external testing.</p>
                </div>
              </div>

              <div className="flex gap-6 group">
                <span className="text-3xl font-black text-gray-100 group-hover:text-[#A11D1D]/20 transition-colors">2.</span>
                <div>
                  <h3 className="text-base font-black text-gray-900 uppercase tracking-wide mb-2">Get in Touch</h3>
                  <p className="text-sm text-gray-500 leading-relaxed font-medium mb-4">Speak to our staff about requirements, duration, format (computer/paper), date, time, and accommodations.</p>
              ...
              <div className="flex gap-6 group">
                <span className="text-3xl font-black text-gray-100 group-hover:text-[#A11D1D]/20 transition-colors">3.</span>
                <div>
          ...
          <div className="lg:col-span-5 flex flex-col gap-8">
            <div className="bg-[#A11D1D] p-10 rounded-xl text-white shadow-xl">
              <h2 className="text-xl font-black uppercase tracking-tight mb-8 flex items-center justify-between text-white">
                Proctoring Fees
                <span className="text-[11px] font-bold text-white/60 tracking-widest uppercase">+ 5% VAT</span>
              </h2>
                  ...
                  {table.getHeaderGroups().map(hg => (
                    <tr key={hg.id} className="border-b border-white/20">
                      {hg.headers.map(header => (
                        <th key={header.id} className={`pb-4 font-black uppercase tracking-widest text-white/50 text-xs ${header.id === 'fee' ? 'text-right' : 'text-left'}`}>
                  ...
                  {table.getRowModel().rows.map(row => (
                    <tr key={row.id} className="border-b border-white/10 hover:bg-white/5 transition-colors ">
                      {row.getVisibleCells().map(cell => (
                        <td key={cell.id} className={`py-5 px-5 ${cell.column.id === 'fee' ? 'text-right' : 'font-bold'}`}>
              ...
              <div className="mt-8 pt-8 border-t border-white/20 space-y-3">
                <div className="flex items-center gap-3 text-xs font-bold text-white/80 uppercase tracking-wide">
                  <CheckCircle2 className="w-4 h-4 text-white" /> One proctor provided
                </div>
                <div className="flex items-center gap-3 text-xs font-bold text-white/80 uppercase tracking-wide">
                  <CheckCircle2 className="w-4 h-4 text-white" /> Per exam, per session
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          ...
          <div className="bg-white p-10 rounded-xl border border-gray-100 shadow-sm">
            <h3 className="text-base font-black text-gray-900 uppercase tracking-widest mb-8 flex items-center gap-3">
              <CreditCard className="w-5 h-5 text-[#A11D1D]" /> 5. Payment Method
            </h3>
            <ul className="space-y-4">
              {["Cash or Card on/before exam day", "Electronic payment via online link", "Wire transfer (Candidate covers charges)"].map((m, i) => (
                <li key={i} className="flex items-center gap-3 text-sm font-medium text-gray-500">
                  <div className="w-1.5 h-1.5 bg-gray-300 rounded-full"></div> {m}
                </li>
              ))}
            </ul>
            <div className="mt-8 p-5 bg-red-50 text-xs font-bold text-[#A11D1D] rounded-lg leading-relaxed">
              Highly recommended to pay in advance to avoid payment failure and inconvenience that could result in cancelling your exam.
            </div>
          </div>
          ...
          <div className="bg-white p-10 rounded-xl border border-gray-100 shadow-sm">
            ...
            <p className="text-sm text-gray-500 leading-relaxed font-medium mb-6">
              Arrive early to settle in and avoid anxiety. Bring an approved valid photo ID and follow home institution regulations.
            </p>
            <div className="flex items-center gap-3 text-xs font-black text-[#A11D1D] uppercase bg-gray-50 p-5 rounded-lg border border-gray-100">
              <Info className="w-4 h-4" /> Double-check ID & belongings before leaving.
            </div>
          </div>
        </div>
      </main>
