...
walkFiles(path.join(process.cwd(), "src"), (filePath) => {
  ...
  for (const [oldName, newName] of Object.entries(componentsToMove)) {
    ...
    const regexRelative = new RegExp(
      `\\.\\./components/${oldBase}(['";x])`,
      "g",
    );
    if (regexRelative.test(content)) {
      content = content.replace(
        regexRelative,
        `../components/blocks/${newBase}$1`,
      );
      changed = true;
    }

    const regexRelative2 = new RegExp(
      `\\.\\./\\.\\./components/${oldBase}(['";x])`,
      "g",
    );
    ...
    if (
      regexLocal.test(content) &&
      filePath.includes("src/components/blocks")
    ) {
